...
def main():
    ...
    for _, hdf5_file in enumerate(hdf5_files):
        ...
        with h5py.File(hdf5_path, 'r') as f:
            ...
            # >>>> [新增 1] 读取触觉数据 >>>>
            # 从 observations 组中读取 tactile
            tactile_data = np.array(f['observations/tactile'])
            # <<<< [结束 1] <<<<
    ...
    for plan_episode in data_list:
        ...
        for gripper_id, gripper in enumerate(grippers):
            ...
            # >>>> [新增 4] 写入 episode_data >>>>
            # 命名为 robotX_tactile，符合 UMI 多机器人命名规范
            episode_data[robot_name + '_tactile'] = tactile.astype(np.float32)
            # <<<< [结束 4] <<<<
    ...
    buffer_index = 0
    directory = config['output_tcp_dir']
